...
15. **时间预算不能乘任何「倾向系数」。** 出发 / 返回时间是用户定的**硬约束**，
    物理时间不会因为选了「紧凑」就多出 15%。曾经 `day_budget` 乘了 `PACE_FACTOR`
    （packed ×1.15），420 分钟的时间窗被算成 483 —— 直接把「最后一天 16:00 返回」
    排成了 19:25（**超时 205 分钟**）。
    倾向的差异只体现在 `PACE_TARGET_MINUTES`（每天目标游玩时长）与 `_flex_scale()`（弹性填充）上。

16. **「地理第一」是靠片区簇落地的，别绕开它。** `cluster_attractions()` 按**直线距离
    ≤1200m** 聚类，簇在分天时**不可拆** —— 这是「锦里古街↔武侯祠（0.24km）必然同日」的保证。
    新增分天逻辑时要保留这条；`llm._validate_days()` 里也有一道同名硬校验盯着。
    另外**别用通行时间做聚类阈值**：速度模型有「1.5km 内一律步行」+「耗时取整到 5 分钟」
    两个噪音，会让阈值在 8~15 分钟之间出现悬崖。

17. **给 ORM 模型加字段，记得往 `core/db.py` 的 `_ADDED_COLUMNS` 加一行。**
