...
**硬编码分天 → LLM 并行写内容 → 硬编码物化时间轴 → LLM 终检。** 这是整个项目的核心设计。

```
① 硬编码分天        瞬时、确定、不花钱
   prune_to_capacity → assign_days → plan_hotels
   地理顺路 + 容量裁剪 + 时段硬知识（熊猫基地排上午这类）都在这里生效

② LLM 并行写内容    每天一路，ThreadPoolExecutor 并发
   每路只产出：当天主题 + 各景点建议 + 餐饮区域与本地特色小吃
...
把宽窄巷子排进两天、把晚餐合并成 14:05 的「下午茶」。提示词里写了两遍都没拦住——
因为「算不算得下」本来就不该由它判断。改成并行流水线后 **17.7 秒，快 3.7 倍**（4 天 10 个景点）。

**时间必须自洽。** `materialize_day()` 内部只有一个 `cur_time` 推进点，所以
`到达 = 上一站离开 + 路程` 恒成立。这条不变量由运行时复核（`service.audit_plan`）和回归脚本双重守着。

**交通按距离分档，纯本地算。** 步行 / 地铁公交 / 打车 / 自驾 / 顺风车 / 城际大巴 / 城际铁路，
按「城区内 / 近郊互通 / 远郊」分档，且分自驾、打车+公共、公共交通三种偏好。
都江堰↔青城山 11.9km 走顺风车 30 分钟——这是真实选择，地铁不通、打车不划算。
**不接路径规划 API**：一次规划十几条腿，打接口既慢又费配额，而规划阶段的 ±10 分钟误差不影响方案可用性。

**餐饮只给区域，不给店名。** 店铺随时会换，写了反而误导；而且为每家店调一次地图接口既慢又费配额。
现在给的是「区域 + 本地特色小吃」，例：`都江堰景区外南桥/西街一带·本地特色午餐 · 推荐：尤兔头、葱葱卷、老腊肉`。

**住宿一次定全程。** 按「全程景点的几何中心」选一个常住片区，只有某天离它超过 45km（比如成都行程里的都江堰日）才建议就近过夜。频繁换酒店是自助游最烦的事之一。

**降级不静默。** 没配 Key 或流水线失败时自动切规则引擎，并在 `summary` 末尾如实注明原因，
前端也会打上「规则引擎生成」标签，不会让人以为这是 AI 排的。

**紧凑度预估前置。** 选景点时就实时告诉用户「这个组合排这几天会超载、会丢掉哪几个」，
纯硬编码、毫秒级返回，不用等到点「开始规划」花 20 秒才知道排不下。
...
| [`docs/BACKEND.md`](docs/BACKEND.md) | 后端符号级地图：每个文件导出什么、行号、三张表结构 |
| [`docs/FRONTEND.md`](docs/FRONTEND.md) | 前端符号级地图：路由、组件、状态、设计系统变量表、响应式 |
| [`docs/API.md`](docs/API.md) | 7 个 HTTP 接口的契约、错误码、紧凑度阈值推导 |
| [`docs/PLAN_SCHEMA.md`](docs/PLAN_SCHEMA.md) | 时间轴数据结构 `PlanResult` 全字段说明 |
| [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md) | 环境搭建、常用命令、回归测试、排查表、待办 |
| [`AGENTS.md`](AGENTS.md) | 给 AI agent 的操作手册与 12 条铁律 |
