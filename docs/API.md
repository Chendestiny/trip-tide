...
`tightness` 判定同时看**负载率**和**舍弃比例**——只看负载率会误判：
...
```
单天预算 per_day_budget = max(180, ((返回时间 - 出发时间) - 两餐 135 分) × 倾向系数)
                          倾向系数：relaxed 0.82 / balanced 1.0 / packed 1.15
容量     capacity        = per_day_budget × 天数          ← 注意：不乘 CAPACITY_SLACK
负载率   load            = (排入的游览总时长 + 折算路程) ÷ capacity
舍弃率   drop_ratio      = 会被舍弃的景点数 ÷ 勾选总数
```

默认 `09:00 → 19:30` + `balanced`：`(630 - 135) × 1.0 = 495 分钟/天`。

⚠️ **两个容易误判的点**：

1. **`load` 对「天数」不敏感**——分子（游览 + 路程）与分母（容量）都随天数近似同比例增长。以成都前 10 个景点为例：

   | 天数 | 容量 | 游览+路程 | `load` | `drop_ratio` | 判定 |
   |---|---|---|---|---|---|
   | 1 天 | 495 | 410 | 0.83 | **0.60**（丢 6/10） | 超载 |
   | 2 天 | 990 | 815 | 0.82 | **0.30**（丢 3/10） | 超载 |
   | 3 天 | 1485 | 1325 | 0.89 | **0.20**（丢 2/10） | 紧凑 |
   | 4 天 | 1980 | 1785 | 0.90 | **0.10**（丢 1/10） | 紧凑 |

   可以看到 `load` 在 0.82~0.90 之间几乎不动，**真正驱动档位变化的是 `drop_ratio`**。所以「加天数」能不能消掉警告，取决于加完之后还有几个景点被舍弃，而不是负载率降了多少。

2. **`preview` 的 `capacity` 不含 `CAPACITY_SLACK`**，而 `planner.prune_to_capacity` 内部乘了 `1.10`。即预估口径比实际裁剪**紧 10%**（同一个方案，预估可能显示装不下、实际排得进去）。

3. 之所以 4 天仍是「紧凑」而不是「适中」：成都这 10 个里有**远郊独占型**景点（都江堰、青城山，单程 56~80 分钟），它们的 `(游览 + 2×单程)` 成本极高，任何一天塞进去都撑爆单天预算，只能被丢或独占整天。**这类行程加天数也难降到「轻松」。**
