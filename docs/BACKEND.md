...
**种子数据**：8 个城市目的地（成都/北京/上海/西安/杭州/重庆/广州/南京，`kind=city`）+ **贵州**（首个 `kind=region` 区域游目的地，20 景 + 8 个过夜基地）≈ 190 条。
成都有 37 条子景点（`trip_spot`），其他城市该表为空。热度为**全国统一 T 级**（0~1000，故宫 990 / 熊猫基地 560）。
