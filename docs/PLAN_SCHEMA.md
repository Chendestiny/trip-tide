...
| `day_plans` | DayPlan[] | 数量恒等于 `days`，`day` 从 1 连续递增（`schemas.py:205` 强校验） |
...
| `advice` | string | 景点是 LLM 写的具体建议；餐饮/住宿是固定文案 |
...
| `must_visit` | bool | `库里 must_visit` **或** `热度 ≥ 90`（`planner.py:744`） |
...
| 位置 | 行号 | 做什么 | 失败后果 |
|---|---|---|---|
| `planner.rule_violations()` | `planner.py:278` | 时段硬约束（熊猫上午 / 夜景 15:00 后 / 博物馆 15:30 前） | 旧工具链会打回；现流水线只记日志 |
| `tools.audit_day()` | `tools.py:525` | 上面 ① ② + 出发时间一致 + 收尾超时 >60 分 | 由 `service.audit_plan` 调用，**只打 warning 日志，不阻断** |
| `scripts/check_planner.py` | — | 8 个用例逐节点跑一遍 6 项校验 | 回归失败要修 |

---

## 饭点是怎么插的

午餐锚点 `11:20`、晚餐锚点 `18:00`，规则（`planner.py:715–802`）：
...
`_apply_day_content()`(`llm.py:236`) 负责把这段 JSON 覆盖到已物化的 `DayPlan` 上——**它只碰文案字段，不碰时间**。
